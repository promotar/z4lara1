...
let ColorInput = { ...Input, ...{
    ...
    events: [
        ["change", "onColorChange", "input"],
		["click", "clearColor", ".vvveb-color-clear"],
	 ],

	onColorChange: function(event, node, input) {
		node.classList.remove("is-empty");
		return input.onChange.call(this, event, node, input);
	},

	clearColor: function(event, node, input) {
		event.preventDefault();
		node.classList.add("is-empty");
		node.dispatchEvent(new CustomEvent("propertyChange", {
			detail: {value: "", input: input, origEvent: event}
		}));
	},

	setValue: function(value) {
		if (this.element[0]) {
			this.element[0].classList.toggle("is-empty", !value);
			let input = this.element[0].querySelector('input');
