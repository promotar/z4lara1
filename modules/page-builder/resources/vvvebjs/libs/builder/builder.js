...
/* drag and drop */
	...
	saveElement: function(element, type, name, callback) {
		...
		fetch(saveReusableUrl, {method: "POST", headers: {'X-CSRF-TOKEN': window.ArtInpaVvvebConfig?.csrfToken || ''}, body: new URLSearchParams(data)})
		.then((response) => {
...
Vvveb.StyleManager = {
	...
	setStyle: function(element, styleProp, value) {
		let clearStyle = value === "" || value === null || typeof value === "undefined";
		...
		if (clearStyle) {
			delete this.styles[media][selector][styleProp];
			if (!Object.keys(this.styles[media][selector]).length) {
				delete this.styles[media][selector];
			}
		} else {
